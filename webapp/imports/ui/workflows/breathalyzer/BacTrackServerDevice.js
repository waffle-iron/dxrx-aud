...
function planBehaviors (behaviors) {
  ...
  _BacTrackServerState.behaviorCursor = 0;
}
...
function executeBehavior (index,plan, successCallback, errorCallback, args) {
  _BacTrackServerState._successCallback[index] = successCallback;
  _BacTrackServerState._errorCallback[index] = errorCallback;
  Meteor.call('bactrackloopback.executeBehaviorAsync',index,plan,args,function(error,result) {
    if (error) {
      console.log('Async call produced error: ' + error + ' and result ' + result);
    }
...
function sim_BacTrackAPIKeyAuthorized (index, step,args) {
...
function sim_BacTrackError (index, step,args) {
  _BacTrackServerState._errorCallbacks[index]('BacTrackError',step.slice(1));
}
