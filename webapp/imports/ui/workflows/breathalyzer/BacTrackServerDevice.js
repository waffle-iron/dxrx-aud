...
var _BacTrackServerStateDispatch = {
  sleep: function(index, step, args) {
    sim_sleep(index, step, args)
  },
  initWithDelegate: function(index, step, args) {
    sim_initWithDelegate(index, step, args)
  },
  connectToNearestBreathalyzer: function(index, step, args) {
    sim_connectToNearestBreathalyzer(index, step, args)
  },
  connectBreathalyzer: function(index, step, args) {
    sim_connectBreathalyzer(index, step, args)
  },
  startScan: function(index, step, args) {
    sim_startScan(index, step, args)
  },
  stopScan: function(index, step, args) {
    sim_stopScan(index, step, args)
  },
  disconnect: function(index, step, args) {
    sim_disconnect(index, step, args)
  },
  startCountdown: function(index, step, args) {
    sim_startCountdown(index, step, args)
  },
  getBreathalyzerBatteryLevel: function(index, step, args) {
    sim_etBreathalyzerBatteryLevel(index, step, args)
  },
  BacTrackAPIKeyDeclined: function(index, step, args) {
    sim_BacTrackAPIKeyDeclined(index, step, args)
  },
  BacTrackAPIKeyAuthorized: function(index, step, args) {
    sim_BacTrackAPIKeyAuthorized(index, step, args)
  },
  BacTrackConnectedOld: function(index, step, args) {
    sim_BacTrackConnectedOld(index, step, args)
  },
  BacTrackConnected: function(index, step, args) {
    sim_BacTrackConnected(index, step, args)
  },
  BacTrackDisconnected: function(index, step, args) {
    sim_BacTrackDisconnected(index, step, args)
  },
  BacTrackCountdown: function(index, step, args) {
    sim_BacTrackCountdown(index, step, args)
  },
  BacTrackStart: function(index, step, args) {
    sim_BacTrackStart(index, step, args)
  },
  BacTrackBlow: function(index, step, args) {
    sim_BacTrackBlow(index, step, args)
  },
  BacTrackAnalyzing: function(index, step, args) {
    sim_BacTrackAnalyzing(index, step, args)
  },
  BacTrackResults: function(index, step, args) {
    sim_BacTrackResults(index, step, args)
  },
  BacTrackError: function(index, step, args) {
    sim_BacTrackError(index, step, args)
  },
  BacTrackConnectTimeout: function(index, step, args) {
    sim_BacTrackConnectTimeout(index, step, args)
  },
  BacTrackGetTimeout: function(index, step, args) {
    sim_BacTrackGetTimeout(index, step, args)
  },
  BacTrackFoundBreathalyzer: function(index, step, args) {
    sim_BacTrackFoundBreathalyzer(index, step, args)
  },
  BacTrackSerial: function(index, step, args) {
    sim_BacTrackSerial(index, step, args)
  },
  BacTrackBatteryLevel: function(index, step, args) {
...
function nextBehavior(functionName, successCallback, errorCallback, args) {
  for (var i = _BacTrackServerState.behaviorCursor; i < _BacTrackServerState.plannedBehaviors
    .length; ++i) {
    if (_BacTrackServerState.plannedBehaviors[i].call.equals(functionName)) {
      _BacTrackServerState.behaviorCursor = i + 1;
      executeBehavior(i, _BacTrackServerState.plannedBehaviors[i],
...
function executeBehavior(index, plan, successCallback, errorCallback, args) {
  ...
  Meteor.call('bactrackloopback.executeBehaviorAsync', index, plan, args,
...
function BacTrackAPIKeyAuthorized(index, step, args) {
  _BacTrackServerState.successCallbacks[index]("BacTrackAPIKeyAuthorized", step
    .slice(1));
}

function sim_BacTrackConnectedOld(index, step, args) {
  _BacTrackServerState.successCallbacks[index]("BacTrackConnectedOld", step.slice(
    1));
}

function sim_BacTrackConnected(index, step, args) {
  _BacTrackServerState.successCallbacks[index]("BacTrackConnected", step.slice(
    1));
}

function sim_BacTrackDisconnected(index, step, args) {
  _BacTrackServerState.successCallbacks[index]("BacTrackDisconnected", step.slice(
    1));
}

function sim_BacTrackCountdown(index, step, args) {
  _BacTrackServerState.successCallbacks[index]("BacTrackCountdown", step.slice(
    1));
}
...
function sim_BacTrackAnalyzing(index, step, args) {
  _BacTrackServerState.successCallbacks[index]("BacTrackAnalyzing", step.slice(
    1));
}
...
function sim_BacTrackConnectTimeout(index, step, args) {
  _BacTrackServerState.successCallbacks[index]("BacTrackConnectTimeout", step.slice(
    1));
}

function sim_BacTrackGetTimeout(index, step, args) {
  _BacTrackServerState.successCallbacks[index]("BacTrackGetTimeout", step.slice(
    1));
}

function sim_BacTrackFoundBreathalyzer(index, step, args) {
  _BacTrackServerState.successCallbacks[index]("BacTrackFoundBreathalyzer",
    step.slice(1));
}
...
function sim_BacTrackBatteryLevel(index, step, args) {
  _BacTrackServerState.successCallbacks[index]("BacTrackBatteryLevel", step.slice(
    1));
}
export function initWithDelegate(successCallback, apiKey, timeoutSeconds) {
  init();
  console.log("In bactrackloopback initwithdelegate");
  nextBehavior("initWithDelegate", successCallback, successCallback, [apiKey,
    timeoutSeconds
  ]);
}
...
export function connectBreathalyzer(preferredUuid, timeoutSeconds,
  successCallback, errorCallback) {
  nextBehavior("connectBreathalyzer", successCallback, errorCallback, [
    preferredUuid, timeoutSeconds
  ]);
}
