//////////////////////////////////////////////////////////////////////////
//                                                                      //
// This is a generated file. You can view the original                  //
// source in your browser if your browser supports source maps.         //
// Source maps are supported by all recent versions of Chrome, Safari,  //
// and Firefox, and by Internet Explorer 11.                            //
//                                                                      //
//////////////////////////////////////////////////////////////////////////


(function () {

/* Imports */
var Meteor = Package.meteor.Meteor;
var global = Package.meteor.global;
var meteorEnv = Package.meteor.meteorEnv;
var Session = Package.session.Session;
var WebApp = Package.webapp.WebApp;
var Log = Package.logging.Log;
var Tracker = Package.deps.Tracker;
var Deps = Package.deps.Deps;
var DDP = Package['ddp-client'].DDP;
var Mongo = Package.mongo.Mongo;
var Blaze = Package.ui.Blaze;
var UI = Package.ui.UI;
var Handlebars = Package.ui.Handlebars;
var Spacebars = Package.spacebars.Spacebars;
var Template = Package.templating.Template;
var check = Package.check.check;
var Match = Package.check.Match;
var _ = Package.underscore._;
var $ = Package.jquery.$;
var jQuery = Package.jquery.jQuery;
var Random = Package.random.Random;
var EJSON = Package.ejson.EJSON;
var FastClick = Package.fastclick.FastClick;
var LaunchScreen = Package['launch-screen'].LaunchScreen;
var HTML = Package.htmljs.HTML;

/* Package-scope variables */
var Header, Theme;

(function(){

///////////////////////////////////////////////////////////////////////
//                                                                   //
// packages/clinical_theming/packages/clinical_theming.js            //
//                                                                   //
///////////////////////////////////////////////////////////////////////
                                                                     //
(function () {                                                       // 1
                                                                     // 2
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//                                                                                                                     //
// packages/clinical:theming/client/helpers.js                                                                         //
//                                                                                                                     //
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                                                                                                                       //
Template.registerHelper("getBackgroundColor", function (paletteColor){                                                 // 1
  if (paletteColor) {                                                                                                  // 2
    return "background-color: " + Theme.getPaletteColor(paletteColor) + "; ";                                          // 3
  } else {                                                                                                             // 4
    return "background-color: " + Theme.getBackgroundColor() + "; ";                                                   // 5
  }                                                                                                                    // 6
});                                                                                                                    // 7
Template.registerHelper("paletteColor", function (color){                                                              // 8
  return "color: " + Theme.getPaletteColor(color) + "; ";                                                              // 9
});                                                                                                                    // 10
Template.registerHelper("getPaletteColor", function (color){                                                           // 11
  return "color: " + Theme.getPaletteColor(color) + "; ";                                                              // 12
});                                                                                                                    // 13
Template.registerHelper("getLinkColor", function (color){                                                              // 14
  if (color) {                                                                                                         // 15
    return "color: " + Theme.getPaletteColor(color) + "; ";                                                            // 16
  } else {                                                                                                             // 17
    return "color: " + Theme.getPaletteColor("colorB") + "; ";                                                         // 18
  }                                                                                                                    // 19
});                                                                                                                    // 20
Template.registerHelper("brandColor", function (color){                                                                // 21
  return "color: " + Theme.getBrandColor(color) + "; ";                                                                // 22
});                                                                                                                    // 23
Template.registerHelper("getBrandColor", function (color){                                                             // 24
  return "color: " + Theme.getBrandColor(color) + "; ";                                                                // 25
});                                                                                                                    // 26
Template.registerHelper("appTitle", function (color){                                                                  // 27
  return Theme.getAppTitle();                                                                                          // 28
});                                                                                                                    // 29
                                                                                                                       // 30
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                                                                     // 40
}).call(this);                                                       // 41
                                                                     // 42
                                                                     // 43
                                                                     // 44
                                                                     // 45
                                                                     // 46
                                                                     // 47
(function () {                                                       // 48
                                                                     // 49
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//                                                                                                                     //
// packages/clinical:theming/objects/Theme.js                                                                          //
//                                                                                                                     //
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                                                                                                                       //
Header = {};                                                                                                           // 1
                                                                                                                       // 2
if (Meteor.isClient) {                                                                                                 // 3
  Session.setDefault('ThemeConfig', {                                                                                  // 4
    appTitle: "",                                                                                                      // 5
    backgroundColor: "",                                                                                               // 6
    backgroundUrl: "",                                                                                                 // 7
    background: {                                                                                                      // 8
      color: "",                                                                                                       // 9
      url: "",                                                                                                         // 10
      images: []                                                                                                       // 11
    },                                                                                                                 // 12
    page: {                                                                                                            // 13
      background: ""                                                                                                   // 14
    },                                                                                                                 // 15
    brand: {                                                                                                           // 16
      primary: "",                                                                                                     // 17
      success: "",                                                                                                     // 18
      info: "",                                                                                                        // 19
      warning: "",                                                                                                     // 20
      danger: ""                                                                                                       // 21
    },                                                                                                                 // 22
    palette: {                                                                                                         // 23
      colorA: "",                                                                                                      // 24
      colorB: "",                                                                                                      // 25
      colorC: "",                                                                                                      // 26
      colorD: "",                                                                                                      // 27
      colorE: ""                                                                                                       // 28
    }                                                                                                                  // 29
  });                                                                                                                  // 30
}                                                                                                                      // 31
                                                                                                                       // 32
                                                                                                                       // 33
if (Meteor.isClient) {                                                                                                 // 34
  Theme = {                                                                                                            // 35
    /**                                                                                                                // 36
     * @summary Configure the global Theme object for the application.                                                 // 37
     * @memberOf Theme                                                                                                 // 38
     * @name configure                                                                                                 // 39
     * @version 1.2.3                                                                                                  // 40
     * @example                                                                                                        // 41
     * ```js                                                                                                           // 42
     * Theme.configure({                                                                                               // 43
     *   appTitle: "Nifty Spiffy App",                                                                                 // 44
     *   backgroundColor: "lightgray",                                                                                 // 45
     *   backgroundUrl: ""                                                                                             // 46
     * });                                                                                                             // 47
     * ```                                                                                                             // 48
     */                                                                                                                // 49
    configure: function (configObject) {                                                                               // 50
      Session.set('ThemeConfig', configObject);                                                                        // 51
      this.paintBackgroundColor();                                                                                     // 52
    },                                                                                                                 // 53
    /**                                                                                                                // 54
     * @summary Get the title for the application.                                                                     // 55
     * @memberOf Theme                                                                                                 // 56
     * @name getAppTitle                                                                                               // 57
     * @version 1.2.3                                                                                                  // 58
     * @example                                                                                                        // 59
     * ```js                                                                                                           // 60
     * Template.appLayout.helpers({                                                                                    // 61
     *   getTitleText: function () {                                                                                   // 62
     *     return Theme.getAppTitle();                                                                                 // 63
     *   }                                                                                                             // 64
     * });                                                                                                             // 65
     * ```                                                                                                             // 66
     */                                                                                                                // 67
    getAppTitle: function (){                                                                                          // 68
      var theme = Session.get('ThemeConfig');                                                                          // 69
      if (theme.appTitle) {                                                                                            // 70
        return theme.appTitle;                                                                                         // 71
      } else {                                                                                                         // 72
        Session.set('errorMessage', "Please configure app theme.");                                                    // 73
        return "";                                                                                                     // 74
      }                                                                                                                // 75
    },                                                                                                                 // 76
    /**                                                                                                                // 77
     * @summary Set the title for the application.                                                                     // 78
      * @memberOf Theme                                                                                                // 79
     * @name setAppTitle                                                                                               // 80
     * @version 1.2.3                                                                                                  // 81
     * @example                                                                                                        // 82
     * ```js                                                                                                           // 83
     * Theme.setAppTitle($('#appTitleInput').val());                                                                   // 84
     * ```                                                                                                             // 85
     */                                                                                                                // 86
    setAppTitle: function (newTitle){                                                                                  // 87
      var theme = Session.get('ThemeConfig');                                                                          // 88
      theme.appTitle = newTitle;                                                                                       // 89
      Session.set('ThemeConfig', theme);                                                                               // 90
    },                                                                                                                 // 91
                                                                                                                       // 92
    setBackgroundUrl: function (newUrl){                                                                               // 93
      var theme = Session.get('ThemeConfig');                                                                          // 94
      theme.background.url = newUrl;                                                                                   // 95
      Session.set('ThemeConfig', theme);                                                                               // 96
    },                                                                                                                 // 97
    /**                                                                                                                // 98
     * @summary Set the background color that the app should use.                                                      // 99
     * @memberOf Theme                                                                                                 // 100
     * @name setBackgroundColor                                                                                        // 101
     * @version 1.2.3                                                                                                  // 102
     * @example                                                                                                        // 103
     * ```js                                                                                                           // 104
     * Theme.setBackgroundColor('blue');                                                                               // 105
     * Theme.setBackgroundColor('green');                                                                              // 106
     * Theme.setBackgroundColor('maroon');                                                                             // 107
     * Theme.setBackgroundColor('gray');                                                                               // 108
     * Theme.setBackgroundColor('lightgray');                                                                          // 109
     * Theme.setBackgroundColor('gradient');                                                                           // 110
     * Theme.setBackgroundColor('background-color: #808080;');                                                         // 111
     * ```                                                                                                             // 112
     */                                                                                                                // 113
    setBackgroundColor: function (newColor){                                                                           // 114
      var theme = Session.get('ThemeConfig');                                                                          // 115
      theme.background.color = newColor;                                                                               // 116
      Session.set('ThemeConfig', theme);                                                                               // 117
      this.paintBackgroundColor();                                                                                     // 118
    },                                                                                                                 // 119
    /**                                                                                                                // 120
     * @summary Triggers a repaint of the body background, using whatever background color value is set in the Theme object.
     * @memberOf Theme                                                                                                 // 122
     * @name paintBackgroundColor                                                                                      // 123
     * @version 1.2.3                                                                                                  // 124
     * @example                                                                                                        // 125
     * ```js                                                                                                           // 126
     * Theme.paintBackgroundColor();                                                                                   // 127
     * ```                                                                                                             // 128
     */                                                                                                                // 129
    paintBackgroundColor: function (){                                                                                 // 130
      //console.log('paintBackgroundColor');                                                                           // 131
                                                                                                                       // 132
      var theme = Session.get('ThemeConfig');                                                                          // 133
      var themeColor = theme.background.color;                                                                         // 134
                                                                                                                       // 135
      switch (themeColor) {                                                                                            // 136
        case 'maroon':                                                                                                 // 137
          $('body').attr('style', "background-color: #A64C4C;  ");                                                     // 138
          break;                                                                                                       // 139
        case 'blue':                                                                                                   // 140
          $('body').attr('style', "background-color: #89cff0;  ");                                                     // 141
          break;                                                                                                       // 142
        case 'green':                                                                                                  // 143
          $('body').attr('style', "background-color: #AEC9A8;  ");                                                     // 144
          break;                                                                                                       // 145
        case 'white':                                                                                                  // 146
          $('body').attr('style', "background-color: #ffffff;  ");                                                     // 147
          break;                                                                                                       // 148
        case 'gray':                                                                                                   // 149
          $('body').attr('style', "background-color: #999999;  ");                                                     // 150
          break;                                                                                                       // 151
        case 'lightgray':                                                                                              // 152
          $('body').attr('style', "background-color: #cccccc;  ");                                                     // 153
          break;                                                                                                       // 154
        case 'medbook':                                                                                                // 155
          $('body').attr("style", "background-size: auto !important; background-repeat: repeat; background-image: url('data:image/png;base64,');");
          break;                                                                                                       // 157
        case 'gradient':                                                                                               // 158
          $('body').attr("style", "background-image: -webkit-linear-gradient(top, #555555, #aaaaaa 100%); background-image: -moz-linear-gradient(top, #555555, #aaaaaa 100%); background-image: -o-linear-gradient(top, #555555, #aaaaaa 100%); -webkit-background-size: cover; -moz-background-size: cover; -o-background-size: cover; background-size: cover; background-repeat: no-repeat; background-attachment: fixed; -moz-user-select: none; -khtml-user-select: none;");
          break;                                                                                                       // 160
        case 'image':                                                                                                  // 161
          $('body').attr('style', "background: url(" + theme.background.url + ") no-repeat center center fixed;");     // 162
          break;                                                                                                       // 163
        default:                                                                                                       // 164
          $('body').attr('style', "background-color: " + themeColor);                                                  // 165
      }                                                                                                                // 166
    },                                                                                                                 // 167
                                                                                                                       // 168
    backgroundColor: function (){                                                                                      // 169
      //console.log('paintBackgroundColor');                                                                           // 170
                                                                                                                       // 171
      var theme = Session.get('ThemeConfig');                                                                          // 172
      var themeColor = theme.background.color;                                                                         // 173
                                                                                                                       // 174
      switch (themeColor) {                                                                                            // 175
        case 'maroon':                                                                                                 // 176
          return "background-color: #A64C4C;";                                                                         // 177
          break;                                                                                                       // 178
        case 'blue':                                                                                                   // 179
          return "background-color: #89cff0;";                                                                         // 180
          break;                                                                                                       // 181
        case 'green':                                                                                                  // 182
          return "background-color: #AEC9A8;";                                                                         // 183
          break;                                                                                                       // 184
        case 'white':                                                                                                  // 185
          return "background-color: #ffffff;";                                                                         // 186
          break;                                                                                                       // 187
        case 'gray':                                                                                                   // 188
          return "background-color: #999999;";                                                                         // 189
          break;                                                                                                       // 190
        case 'lightgray':                                                                                              // 191
          return "background-color: #cccccc;";                                                                         // 192
          break;                                                                                                       // 193
        case 'medbook':                                                                                                // 194
          return "background-size: auto !important; background-repeat: repeat; background-image: url('data:image/png;base64,');";
          break;                                                                                                       // 196
        case 'gradient':                                                                                               // 197
          return "background-image: -webkit-linear-gradient(top, #555555, #aaaaaa 100%); background-image: -moz-linear-gradient(top, #555555, #aaaaaa 100%); background-image: -o-linear-gradient(top, #555555, #aaaaaa 100%); -webkit-background-size: cover; -moz-background-size: cover; -o-background-size: cover; background-size: cover; background-repeat: no-repeat; background-attachment: fixed; -moz-user-select: none; -khtml-user-select: none;";
          break;                                                                                                       // 199
        default:                                                                                                       // 200
          return "background-color: " + themeColor + ";";                                                              // 201
      }                                                                                                                // 202
    },                                                                                                                 // 203
                                                                                                                       // 204
    /**                                                                                                                // 205
     * @summary Set the brand color that the app should use.                                                           // 206
     * @memberOf Theme                                                                                                 // 207
     * @name setBrandColors                                                                                            // 208
     * @version 1.2.3                                                                                                  // 209
     * @example                                                                                                        // 210
     * ```js                                                                                                           // 211
     * Theme.setBrandColor('#2C302E', '#909590', '#256D1B', '#9AE19D', '#537A5A');                                     // 212
     * ```                                                                                                             // 213
     */                                                                                                                // 214
    setBrandColors: function (colorA, colorB, colorC, colorD, colorE){                                                 // 215
      var theme = Session.get('ThemeConfig');                                                                          // 216
      theme.brand = {                                                                                                  // 217
        primary: colorA,                                                                                               // 218
        success: colorB,                                                                                               // 219
        info: colorC,                                                                                                  // 220
        warning: colorD,                                                                                               // 221
        danger: colorE                                                                                                 // 222
      };                                                                                                               // 223
      Session.set('ThemeConfig', theme);                                                                               // 224
      this.paintBackgroundColor();                                                                                     // 225
    },                                                                                                                 // 226
    /**                                                                                                                // 227
     * @summary Set the brand color that the app should use.                                                           // 228
     * @memberOf Theme                                                                                                 // 229
     * @name setBrandColors                                                                                            // 230
     * @version 1.2.3                                                                                                  // 231
     * @example                                                                                                        // 232
     * ```js                                                                                                           // 233
     * Theme.setBrandColor('#2C302E', '#909590', '#256D1B', '#9AE19D', '#537A5A');                                     // 234
     * ```                                                                                                             // 235
     */                                                                                                                // 236
    setPaletteColors: function (colorA, colorB, colorC, colorD, colorE){                                               // 237
      var theme = Session.get('ThemeConfig');                                                                          // 238
      theme.palette = {                                                                                                // 239
        colorA: colorA,                                                                                                // 240
        colorB: colorB,                                                                                                // 241
        colorC: colorC,                                                                                                // 242
        colorD: colorD,                                                                                                // 243
        colorE: colorE                                                                                                 // 244
      };                                                                                                               // 245
      Session.set('ThemeConfig', theme);                                                                               // 246
      this.paintBackgroundColor();                                                                                     // 247
    },                                                                                                                 // 248
    /**                                                                                                                // 249
     * @summary Get the brand color that the app should use.  Accepts one of [primary, success, info, warning, danger] // 250
     * @memberOf Theme                                                                                                 // 251
     * @name getBrandColor                                                                                             // 252
     * @version 1.2.3                                                                                                  // 253
     * @example                                                                                                        // 254
     * ```js                                                                                                           // 255
     * Theme.getBrandColor('success');                                                                                 // 256
     * ```                                                                                                             // 257
     */                                                                                                                // 258
    getBrandColor: function (key){                                                                                     // 259
      var theme = Session.get('ThemeConfig');                                                                          // 260
      return theme.brand[key];                                                                                         // 261
    },                                                                                                                 // 262
    /**                                                                                                                // 263
     * @summary Get the palette color that the app should use.  Accepts one of [colorA, colorB, colorC, colorD, colorE]
     * @memberOf Theme                                                                                                 // 265
     * @name getPaletteColor                                                                                           // 266
     * @version 1.2.3                                                                                                  // 267
     * @example                                                                                                        // 268
     * ```js                                                                                                           // 269
     * Theme.getPaletteColor('colorA');                                                                                // 270
     * ```                                                                                                             // 271
     */                                                                                                                // 272
    getPaletteColor: function (key){                                                                                   // 273
      var theme = Session.get('ThemeConfig');                                                                          // 274
      return theme.palette[key];                                                                                       // 275
    },                                                                                                                 // 276
    /**                                                                                                                // 277
     * @summary Get the background color the app should use.                                                           // 278
     * @memberOf Theme                                                                                                 // 279
     * @name getBackgroundColor                                                                                        // 280
     * @version 1.2.3                                                                                                  // 281
     * @example                                                                                                        // 282
     * ```js                                                                                                           // 283
     * Theme.getBackgroundColor();                                                                                     // 284
     * ```                                                                                                             // 285
     */                                                                                                                // 286
    getBackgroundColor: function (key){                                                                                // 287
      var theme = Session.get('ThemeConfig');                                                                          // 288
      if (key) {                                                                                                       // 289
        return theme.palette[key];                                                                                     // 290
      } else {                                                                                                         // 291
        return theme.background.color;                                                                                 // 292
      }                                                                                                                // 293
    }                                                                                                                  // 294
  };                                                                                                                   // 295
                                                                                                                       // 296
}                                                                                                                      // 297
                                                                                                                       // 298
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                                                                     // 355
}).call(this);                                                       // 356
                                                                     // 357
///////////////////////////////////////////////////////////////////////

}).call(this);


/* Exports */
if (typeof Package === 'undefined') Package = {};
(function (pkg, symbols) {
  for (var s in symbols)
    (s in pkg) || (pkg[s] = symbols[s]);
})(Package['clinical:theming'] = {}, {
  Theme: Theme
});

})();
